/* ================================================================
   HIMAL THAPA – PORTFOLIO SCRIPT
   Features: Loader, Cursor, Particles, Typed, Nav, Skills,
   Portfolio Filter, Counters, Testimonials, Certifications,
   Contact Form, Deep Links, AOS, Theme Toggle,
   *** BILINGUAL LANGUAGE SYSTEM (Nepali / English) ***
================================================================ */

/* ── SOCIAL CONFIG ────────────────────────────────────────── */
const SOCIAL = {
  facebook:    'https://facebook.com/thimal522',
  youtube:     'https://www.youtube.com/@HimalThapa',
...
const TRANSLATIONS = {
  np: {
    ...
    /* Hero */
    hero_greeting: 'नमस्ते दुनियाँ, म हुँ',
    hero_name_first: 'हिमल',
    hero_name_last: 'थापा',
    ...
    /* Lang toggle label (shown when NP is active, clicking will switch to EN) */
    lang_btn_label: 'EN',
  },
  ...
  en: {
    ...
    hero_name_first: 'Himal',
    hero_name_last: 'Thapa',
    ...
    skills_label: 'What I Know', skills_title_my: 'My', skills_title: 'Skills',
    skill_cat_frontend: 'Frontend', skill_cat_backend: 'Backend',
    skill_cat_db: 'Database', skill_cat_tools: 'Tools',
    ...
    exp3_desc:  'Delivered custom web solutions for local businesses, improving their digital presence and operational efficiency through tailored software.',
    ...
    edu3_desc:  'Completed SEE (School Leaving Certificate) with excellent academic performance. Participated in various IT and science competitions.',
    edu3_badge: '🎯 SEE Passed',

    cert_label: 'Credentials', cert_title_my: 'My', cert_title: 'Certifications',
    cert_modal_note: 'Certificate preview coming soon. Replace with actual certificate image.',
    ...
    test2_text: 'Professional, reliable, and incredibly talented. Himal transformed our outdated website into a modern masterpiece that our customers love.',
    ...
    test4_text: 'Highly recommended! Himal\'s IT consulting services helped us streamline our operations and reduce costs significantly.',
...
/* ================================================================
   LANGUAGE SYSTEM
================================================================ */
let currentLang = localStorage.getItem('ht-lang') || 'np';

function t(key) {
  return TRANSLATIONS[currentLang][key] || TRANSLATIONS['np'][key] || key;
}
...
function applyTranslations(lang) {
  ...
  // Special: hero name stays stylized (span structure)
  const heroName = document.querySelector('.hero-name');
  if (heroName) {
    heroName.innerHTML = lang === 'np'
      ? 'हिमल<br /><span class="gradient-text">थापा</span>'
      : 'Himal<br /><span class="gradient-text">Thapa</span>';
  }

  // About title (has nested span)
  const aboutTitle = document.querySelector('.section-title[data-i18n="about_title"]');
  if (aboutTitle) {
    aboutTitle.innerHTML = TRANSLATIONS[lang].about_title +
  ...
  // Update form validation messages reference lang
  currentValidationLang = lang;

  // Stats section: Nepali numerals vs Arabic
  updateStatNumerals(lang);
}

function updateStatNumerals(lang) {
  // The counters use JS animation, just re-run with correct suffix
  // No action needed since counter values are numeric; they show the same digits.
}
...
window.addEventListener('load', () => {
  ...
  if (!loader) return;
  // Apply lang before hiding so loader text is correct
  applyTranslations(currentLang);
...
/* ================================================================
   4. NAVBAR — sticky, active links, hamburger
================================================================ */
(function initNav() {
...
/* ================================================================
   7. TYPED ANIMATION — restartable for language switch
================================================================ */
let typedTimer = null;
let typedWords = TRANSLATIONS[currentLang].typed_words;
...
function startTyped() {
  ...
  function type() {
    ...
    else if (deleting && ci < 0) { deleting = false; ci = 0; wi = (wi + 1) % typedWords.length; delay = 280; }
    typedTimer = setTimeout(type, delay);
  }
  typedTimer = setTimeout(type, 500);
}
...
(function initParticles() {
  ...
  class Particle {
    ...
    draw() {
      ...
      ctx.fillStyle = isDark() ? `rgba(0,229,255,${this.alpha})` : `rgba(108,99,255,${this.alpha * 0.5})`;
      ctx.fill();
    }
  }
      ...
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (d < dist) {
          ...
          ctx.strokeStyle = isDark()
            ? `rgba(108,99,255,${0.15 * (1 - d / dist)})`
            : `rgba(108,99,255,${0.08 * (1 - d / dist)})`;
  ...
  canvas.parentElement && canvas.parentElement.addEventListener('mousemove', e => {
...
(function initTestimonials() {
  ...
  const dotsBox = document.getElementById('tl-dots');
  if (!track) return;
  ...
  function go(i) {
    ...
    document.querySelectorAll('.tl-dot').forEach((d, j) => d.classList.toggle('active', j === index));
  }
  function startAuto() { auto = setInterval(() => go(index + 1), 4500); }
  function stopAuto()  { clearInterval(auto); }
  prevBtn && prevBtn.addEventListener('click', () => { stopAuto(); go(index - 1); startAuto(); });
  nextBtn && nextBtn.addEventListener('click', () => { stopAuto(); go(index + 1); startAuto(); });
  startAuto();
  let touchX = 0;
  track.addEventListener('touchstart', e => { touchX = e.touches[0].clientX; }, { passive: true });
  track.addEventListener('touchend',   e => {
    const diff = touchX - e.changedTouches[0].clientX;
    if (Math.abs(diff) > 40) { stopAuto(); go(index + (diff > 0 ? 1 : -1)); startAuto(); }
  }, { passive: true });
})();


/* ================================================================
   13. CERTIFICATION MODAL
================================================================ */
const certs = [
  { icon: 'fab fa-aws',        title: 'AWS Cloud Practitioner',     issuer: 'Amazon Web Services',      year: '2023' },
  { icon: 'fab fa-google',     title: 'Google IT Support',           issuer: 'Google / Coursera',         year: '2023' },
  { icon: 'fab fa-react',      title: 'React Developer Certificate', issuer: 'Meta / Coursera',           year: '2022' },
  { icon: 'fab fa-node-js',    title: 'Node.js Certification',       issuer: 'OpenJS Foundation',         year: '2022' },
  { icon: 'fas fa-shield-alt', title: 'Cybersecurity Essentials',    issuer: 'Cisco Networking Academy',  year: '2021' },
  { icon: 'fab fa-git-alt',    title: 'Git & GitHub Mastery',        issuer: 'Udemy',                     year: '2021' },
];
...
function openCertModal(i) {
  ...
  if (!modal || !certs[i]) return;
  iconEl.innerHTML     = `<i class="${certs[i].icon}"></i>`;
  titleEl.textContent  = certs[i].title;
  issuerEl.textContent = certs[i].issuer;
  yearEl.textContent   = certs[i].year;
...
(function initForm() {
  ...
  function validate() {
    ...
    const L = TRANSLATIONS[currentValidationLang];

    if (!name || name.length < 2) { setError('name', L.err_name); ok = false; }
    else setError('name', '');
    const emailReg = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!email || !emailReg.test(email)) { setError('email', L.err_email); ok = false; }
    else setError('email', '');
    if (!subject || subject.length < 3) { setError('subject', L.err_subject); ok = false; }
    else setError('subject', '');
    if (!message || message.length < 10) { setError('message', L.err_message); ok = false; }
  ...
  form.addEventListener('submit', e => {
    ...
    const L = TRANSLATIONS[currentValidationLang];
    if (btnText) btnText.innerHTML = `<i class="fas fa-spinner fa-spin"></i> ${L.sending}`;
    setTimeout(() => {
      if (btnText) btnText.innerHTML = `<i class="fas fa-paper-plane"></i> <span data-i18n="form_send">${L.form_send}</span>`;
      if (success) { success.style.display = 'flex'; success.querySelector('[data-i18n="form_success"]') && (success.querySelector('[data-i18n="form_success"]').textContent = L.form_success); }
      form.reset();
...
/* ================================================================
   15. SOCIAL DEEP LINKS
================================================================ */
(function initDeepLinks() {
  function setHrefs(selector, url) {
    document.querySelectorAll(selector).forEach(el => {
      if (el.href && el.href.includes('_URL')) el.href = url;
    });
  }
  setHrefs('[href="FACEBOOK_URL"]',  SOCIAL.facebook);
  setHrefs('[href="YOUTUBE_URL"]',   SOCIAL.youtube);
  setHrefs('[href="TIKTOK_URL"]',    SOCIAL.tiktok);
  setHrefs('[href="INSTAGRAM_URL"]', SOCIAL.instagram);
  setHrefs('[href="GITHUB_URL"]',    SOCIAL.github);
  setHrefs('[href="LINKEDIN_URL"]',  SOCIAL.linkedin);
  setHrefs('[href="mailto:EMAIL_ADDRESS"]', 'mailto:' + SOCIAL.email);

  const waUrl = `https://wa.me/${SOCIAL.whatsapp.replace(/\D/g,'')}?text=${encodeURIComponent(SOCIAL.whatsappMsg)}`;
  ...
  document.querySelectorAll('[data-app]').forEach(el => {
    el.addEventListener('click', function(e) {
      ...
      let appUri = null;
      if (app === 'fb')        appUri = 'fb://facewebmodal/f?href=' + encodeURIComponent(url);
      if (app === 'instagram') appUri = 'instagram://user?username=YOUR_USERNAME';
      if (app === 'youtube')   appUri = 'vnd.youtube://';
      if (app === 'tiktok')    appUri = 'snssdk1233://';
      if (!appUri) return;
      e.preventDefault();
      const start = Date.now();
      window.location = appUri;
      setTimeout(() => { if (Date.now() - start < 2000) window.open(url, '_blank'); }, 1200);
    });
  });
})();
...
(function initMagnetic() {
  document.querySelectorAll('.magnetic').forEach(btn => {
    btn.addEventListener('mousemove', function(e) {
      ...
      this.style.transform = `translate(${mx * 0.18}px, ${my * 0.18}px)`;
    });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Init language toggle after DOM ready
  initLangToggle();
});
...
(function initParallax() {
  ...
  window.addEventListener('mousemove', e => {
    ...
    shapes.forEach((s, i) => {
      ...
      s.style.transform = `translate(${cx * factor}px, ${cy * factor}px)`;
    });
  }, { passive: true });
})();
